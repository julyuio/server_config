...
DEST_FILE="${DEST_DIR}/dbip-city.mmdb.gz"
DEST_FILE_nogz="${DEST_DIR}/dbip-city.mmdb"
...
read response
  if [ "$response" = "y" ] || [ -z "$response" ]; then
	...
	(crontab -l 2>/dev/null; echo "28 * * * * $GOACC_REPORT_NO404") | crontab -

  else
            echo "Please add them manually... "
	    echo -e " ${IGreen} 26 * * * * $GOACC_REPORT ${Color_Off}"
	    echo -e " ${IGreen} 26 * * * * $GOACC_REPORT_NO404 ${Color_Off}"
  fi
